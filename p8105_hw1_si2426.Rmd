...
flippervsbill_plot <- ggplot(penguins, aes(x=bill_length_mm, y=flipper_length_mm, color=species)) + geom_point()
...
The numeric vector has a mean of `r problem2_df %>% pull(numvec) %>% mean()`.
The logical vector has a mean of `r problem2_df %>% pull(logvec) %>% mean()`.
The character vector has a mean of `r problem2_df %>% pull(charvec) %>% mean()`, which indicates a mean cannot be taken.
The factor vector has a mean of `r problem2_df %>% pull(facvec) %>% mean()`, which indicates a mean cannot be taken.

We are able to find the mean of the numeric and logical vectors. However, we are unable to find the means of the character and factor vectors.
